...
#
# # Проверка наличия товара для добавления в корзину
...
#         )
#
# # Функция поиска товара в корзине текущего пользователя по product_id
# async def _get_cart_item(
#     db: AsyncSession, user_id: int, product_id: int
# ) -> CartItemModel | None:
#     result = await db.scalars(
#         select(CartItemModel)
#         .options(selectinload(CartItemModel.product))    # стратегия "жадной" загрузки связанных данных
#         .where(                                          # в SQLAlchemy, которая выполняет дополнительный запрос с IN
...
#     result = await db.scalars(
#         select(CartItemModel)
#         .options(selectinload(CartItemModel.product))           # "жадная" загрузка
